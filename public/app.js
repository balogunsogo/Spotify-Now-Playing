...
async function pollTrack(generation = pollGeneration) {
  window.clearTimeout(pollTimer);
  pollTimer = null;
  await loadTrack();
  if (!document.hidden && isPageActive && generation === pollGeneration) {
    pollTimer = window.setTimeout(() => pollTrack(generation), pollIntervalMs);
  }
}
