...
function normalizeTrack(track, state, playedAt = null, progressMs = null) {
  if (!track) {
    return null;
  }
...
export async function getRecentlyPlayed(limit = 4) {
  const accessToken = await getAccessToken();
  const recent = await spotifyGet("me/player/recently-played?limit=20", accessToken);
  const tracks = [];
  const seen = new Set();
  ...
  for (const item of recent?.items || []) {
    ...
    const track = normalizeTrack(item.track, "recent", item.played_at);
    const identity = `${track.title.trim().toLowerCase()}|${track.artists.trim().toLowerCase()}`;
...
export async function getListeningStatus() {
  ...
  // Currently-playing can return no content for a newly paused session even
  // though the playback-state endpoint still exposes the paused item.
  const playback = await spotifyGet("me/player?additional_types=track", accessToken);
